...
<div className="input-group">
          ...
          <div className="input-group">
            ...
            <textarea
className={`input-field textarea-field${fieldError('Body') || fieldError('body') ? ' input-error' : ''}`}              rows={5}
              value={content}
...
{(fieldError('Body') || fieldError('body')) && (
          ...
          <Button type="submit" variant="primary" isLoading={ui.kind === 'loading'} leftIcon={<Send size={16} />}>            Dispatch Notification Now
          </Button>
        </form>
      </div>
    </div>
