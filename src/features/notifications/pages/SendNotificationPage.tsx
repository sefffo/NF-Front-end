...
// ─── Helper: parse comma/semicolon/newline separated addresses ────────────────
...
export const SendNotificationPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // ── EMAIL channel → call real backend API ─────────────────────────────────
    if (channel === 'EMAIL') {
      ...
      if (result.ok) {
        setUi({
          kind: 'success',
          message: `Email sent successfully. Message ID: ${result.data.messageId}`,
        });
      ...
      if (result.status === 422 || result.status === 403) {
        setUi({ kind: 'domain', title: result.title, detail: result.detail });
        return;
      }
    ...
    // ── Other channels (SMS / PUSH / WEBHOOK) → keep existing mock ────────────
    try {
      ...
      setUi({
        kind: 'success',
        message: `Notification successfully queued & dispatched! Tracking ID: ${res.id}`,
      });
  ...
  const fieldError = (field: string): string | undefined => {
    ...
    const msgs = ui.errors[field] ?? ui.errors[field.toLowerCase()] ?? ui.errors[field.toUpperCase()];
    return msgs?.join(', ');
  };
    ...
    <div className="page-container-inner">
      ...
      <div className="card card-padded max-w-2xl">
        <form onSubmit={handleSubmit} className="form-stack">
          ...
          {/* Recipient — for EMAIL, allow multiple comma-separated */}
          <div className="input-group">
            <label className="input-label">
              {channel === 'EMAIL'
                ...
                : 'Target Token / URL'}
              {channel === 'EMAIL' && (
                <span className="text-muted text-xs ml-1">(comma-separated for multiple)</span>
              )}
            </label>
            <Input
              value={recipient}
          ...
          <div className="input-group">
            ...
            <textarea
              className={`input-field textarea-field${
                fieldError('Body') || fieldError('body') ? ' input-error' : ''
              }`}
              rows={5}
              value={content}
          ...
          {/* isHtml toggle — only relevant for EMAIL */}
          {channel === 'EMAIL' && (
