...
// ─── Quota bar ────────────────────────────────────────────────────────────────
const QuotaBar: React.FC<{ used: number; max: number; label: string }> = ({
  used,
  max,
  label,
}) => {
...
export const TenantDetailsPage: React.FC = () => {
  ...
  // TenantDto matches GET /api/tenants/{id} response exactly per the OpenAPI spec
        ...
        .catch((err: unknown) => {
          if (axios.isAxiosError(err)) {
            ...
            if (status === 404) setError('Tenant not found.');
              ...
              setError(
                `Failed to load tenant details. (HTTP ${
      ...
      <div className="stats-grid" style={{ marginBottom: 'var(--space-6)' }}>
        <div className="stat-card">
          ...
          <div className="stat-content">
            ...
            <StatusBadge status={tenant.status.toUpperCase()} />          </div>
        </div>
